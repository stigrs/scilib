...
TEST(TestLinalg, TestEighReal)
{
    ...
    // clang-format on
    Sci::Matrix<double> evec(Kokkos::dextents<Sci::index, 2>(5, 5), evec_data);

    Sci::Matrix<double> a(Kokkos::dextents<Sci::index, 2>(5, 5), a_data);
...
TEST(TestLinalg, TestEigColMajor)
{
    ...
    Matrix<double, Kokkos::layout_left> evec_re(extents_type(4, 4), evec_re_data);
    Matrix<double, Kokkos::layout_left> evec_im(extents_type(4, 4), evec_im_data);
    Matrix<double, Kokkos::layout_left> a(extents_type(4, 4), a_data);

    Vector<std::complex<double>, Kokkos::layout_left> eval(4);
    Matrix<std::complex<double>, Kokkos::layout_left> evec(4, 4);
